...
@Component({
  ...
  template: `
    <div class="space-y-4">
      ...
      @if (state.error()) {
        <div class="mx-5 p-4 bg-red-50 text-red-700 rounded">
          {{ state.error() }}
        </div>
      }
...
export class MovimientosComponent implements OnInit {
  ...
  readonly state = inject(MovimientosState);

  readonly tipo = signal<'todos' | 'ingresos' | 'egresos' | 'descuentos'>('todos');

  readonly movimientos = computed(() =>
    this.state.getMovimientosByTipo(this.tipo())
  );

  constructor() {
    // Effect para reaccionar a cambios en el tipo de ruta
    effect(() => {
      const currentTipo = this.tipo();
      // Este effect se ejecutará cada vez que cambie el tipo
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      this.tipo.set(tipoParam || 'todos');
    });

    // Cargar movimientos iniciales
    this.state.loadMovimientos();
  }

  onFiltrosChange(event: { secuencia: number; vigencia: string; destino: string }): void {
    this.state.updateParameters(event);
  }
